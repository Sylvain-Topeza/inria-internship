...
def remove_repeats_paper(dataset, threshold=.95): # costly in memory, as used in the paper
...
def remove_repeats_coeff(dataset, threshold=0.95): # better suited for smaller RAM, but very slow
    ...
    #compute correlatopn between all images in the set
    print('computing correlations')
    if len(dataset_down.shape) != 4 :
        raise ValueError('Input shape error')
    remove_im_mean = lambda data : data - data.mean(dim=(1,2,3),keepdims=True)
    dataset_down = remove_im_mean(dataset_down)
    norms = dataset_down.norm(dim=(2,3), keepdim=True).norm(dim=1, keepdim=True)
    norms[norms == 0] = 0.001
    ...
    # find images with correlations above the threshold
    print('finding repeated images')
    for i in tqdm(range(N), desc='Searching for correlations'):
        if i in to_remove:
            continue
        if i > 0:
            corrs = torch.matmul(dataset_down[i], dataset_down[:i].T)
            if torch.any(torch.abs(corrs) > threshold):
                to_remove.add(i)
        if i % 100 == 0:
            print(f"{i}/{N} processed images, {len(to_remove)} repeated images found")
    #drop one in each pair of repeated images
    print('removing repeated images')
    print(f"Total number of images to remove : {len(to_remove)}")
    keep_indices = [i for i in range(N) if i not in to_remove]
    data_cleaned = dataset[keep_indices]
    return data_cleaned
...
def remove_repeats_block(dataset, threshold=0.95): # better suited for smaller RAM, and faster
    ...
    print('finding repeated images')
    b = 5000 
    for i in tqdm(range(0, N, b), desc='Searching for correlations'):
        bi = min(b, N-i)
        for j in range(0, i+1, b):
            bj = min(b,N-j)
            corrs = torch.matmul(dataset_down[i:i+bi], dataset_down[j:j+bj].T)
            rep_IDs_1 , rep_IDs_2 = torch.where(torch.abs(corrs)>threshold)
            rep_IDs_1 = rep_IDs_1 + i
            rep_IDs_2 = rep_IDs_2 + j
            for k in range(len(rep_IDs_1)):
                if rep_IDs_1[k] == rep_IDs_2[k]:
                    continue
                m = max(rep_IDs_1[k], rep_IDs_2[k])
                if m not in to_remove:
                    to_remove.append(m) # if the correlation is above the threshold, add the image to the set to remove
        print(f"{i}/{N} processed images, {len(to_remove)} repeated images found")
    #drop one in each pair of repeated images
    print('removing repeated images')
    print(f"Total number of images to remove : {len(to_remove)}")
